...
public class WrongEndPointThreadImpl extends Thread{
	
    private String workingDir;
    private String uuid ;
    private String dir;
    private static final Log logger = LogFactory.getLog(WrongEndPointThreadImpl.class);
    private AnalyticsDataService analyticsDataService;
    private int MAX_VALUE = 1000;
    WrongEndPointReportComponents rC;
    private String randomNumber ;
    Random random ;
  
    
    
	public WrongEndPointThreadImpl(WrongEndPointReportComponents rc) {
		
		this.rC = rc;
		uuid = UUID.randomUUID().toString();
		workingDir = System.getProperty("user.dir");
		dir= "/repository/deployment/server/jaggeryapps/portal/tmp/WrongEndpoints/";
		File WrongEndpointsDir = new File(workingDir+dir);
		if (!WrongEndpointsDir.exists()) {
			boolean result = false;

			try{

				result = WrongEndpointsDir.mkdir();
			} 
			catch(SecurityException se){
				logger.error("mkdir failed. need to create /repository/deployment/server/jaggeryapps/portal/tmp/WrongEndpoints/ dir manually");
			}        
			if(result) {    
				logger.info(" /repository/deployment/server/jaggeryapps/portal/tmp/WrongEndpoints/ dir created"); 
			}else {
				logger.error("mkdir failed. need to create /repository/deployment/server/jaggeryapps/portal/tmp/WrongEndpoints/ dir manually");
			}
		}
    ...
    private void writeRecordsToFile(List<Record> records, File file) throws Exception {
        ...
        try {
            ...
            bufferedWriter.write(rC.getTableName()+ rC.getSearchParams()+rC.getTenantId());
            for (Record record : records) {
                StringBuilder stringBuilder = new StringBuilder();
                String comma=",";
                stringBuilder.append(record.getValues().get("datetime").toString());
                ...
                stringBuilder.append(comma);
                stringBuilder.append(record.getValues().get("MNO_lookup").toString());
