var view = {
    ...
    subscriptions: [{
        ...
        callback: function(topic, data, subscriberData) {
            ...
            client.getOffNetRegistrationsPerDay(filter, function (response) {
                var results = JSON.parse(response.message);
                var offNetAuthorizations = [];
                for (var i = 1; i < results.length; i +=2) {
                    offNetAuthorizations.push(results[i]);
                }
                wso2gadgets.onDataReady(offNetAuthorizations);
            }, function (msg) {
    ...
    data: function() {
        var SERVER_URL = "/portal/apis/telcoanalytics";
        var client = new TelcoAnalyticsClient().init(SERVER_URL);
        client.getOffNetRegistrationsPerDay({}, function (response) {
            var results = JSON.parse(response.message);
            var offNetAuthorizations = [];
            for (var i = 1; i < results.length; i +=2) {
                offNetAuthorizations.push(results[i]);
